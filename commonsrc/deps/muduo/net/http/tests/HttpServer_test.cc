...
void ThreadServer(uint16_t port)
{
	  EventLoop temploop ;
	  HttpServer server(&temploop, InetAddress(port), "dummy");
...
int main1(int argc, char* argv[])
{
  ...
  //muduo::Thread t1(boost::bind(ThreadServer, 28080));
 // muduo::Thread t2(boost::bind(ThreadServer, 28081));
 // t1.start();
  ThreadServer(numThreads);
  //t2.start();
  //t1.join();
 //t2.join();
...
int main(int argc, char* argv[])
{
  ...
  EventLoop loop;
  HttpServer server(&loop, InetAddress(28080), "dummy");
  server.setHttpCallback(onRequest);
