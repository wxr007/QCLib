...
void onRequest(ParserPtr parser ,const HttpRequest& req,HttpResponse* resp)
{
	if(!parser)
	{
		return;
...
void onRequest2(ParserPtr parser ,const HttpRequest& req,HttpResponse* resp)
{
  if (req.path() == "/")
  {
    resp->setStatusCode(HttpResponse::k200Ok);
...
int main(int argc, char* argv[])
{
  ...
  EventLoop loop;
  MyHttpServer server(&loop, InetAddress(28080), "dummy");
